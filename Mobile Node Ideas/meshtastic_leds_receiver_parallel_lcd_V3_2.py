...
def main():
    ...
    if GPIO_AVAILABLE:
        try:
            # Clear any existing event detection
            try:
                GPIO.remove_event_detect(BUTTON_PIN)
                print(f"  Cleared existing button event detection")
            except:
                pass
            
            # Test button state first
            button_state = GPIO.input(BUTTON_PIN)
            print(f"  Button initial state: {button_state} ({'HIGH/not pressed' if button_state else 'LOW/pressed'})")
            
            # Add event detection with increased debounce
            GPIO.add_event_detect(
                BUTTON_PIN, 
                GPIO.FALLING,  # Trigger on button press (HIGH to LOW)
                callback=button_callback,
                bouncetime=500  # Increased to 500ms
            )
            print(f"✓ Button interrupt configured on GPIO {BUTTON_PIN} (debounce: 500ms)")
        except Exception as e:
