...
class LCDDisplay:
    ...
    def scroll_text_simple(self, line1, line2="", cycles=3):
        """
        Scroll long text - SIMPLIFIED non-threaded version
        Blocks but actually works with LCD hardware
        """
        if not self.enabled or not self.lcd:
            print("LCD scroll: not enabled or no lcd")
            return
        
        try:
            print(f"LCD scroll_text_simple: line1={len(line1)}ch, line2={len(line2)}ch")
            ...
            if not line1_needs_scroll and not line2_needs_scroll:
                ...
                print("LCD scroll: No scroll needed, static display")
            ...
            print(f"LCD scroll: max_scroll={max_scroll}, cycles={cycles}")
            
            # Scroll multiple times
            for cycle in range(cycles):
                for scroll_pos in range(max_scroll + 5):  # +5 for pause at end
                    try:
                        ...
                        if line1_needs_scroll:
                            pos1 = min(scroll_pos, max_scroll1)
                        ...
                        if line2_needs_scroll:
                            pos2 = min(scroll_pos, max_scroll2)
                        ...
                        print(f"LCD scroll frame error: {e}")
                        break
                
            print("LCD scroll: Complete")
    ...
    def show_alert(self, alert_type, full_message, scroll=True):
        ...
        """
        print(f"LCD.show_alert called: type={alert_type}, msg_len={len(full_message)}, scroll={scroll}")
        
        if not self.enabled or not self.lcd:
            print("LCD show_alert: LCD not enabled or not initialized")
            return
        ...
        # Remove LED command from display
        if "||LED:" in message_preview:
            message_preview = message_preview.split("||LED:")[0].strip()
        
        print(f"LCD Display - Line1: '{line1}', Line2 length: {len(message_preview)}")
        
        if scroll and len(message_preview) > 16:
            print(f"Calling scroll_text_simple (message: {len(message_preview)} chars)")
            # Use simplified scrolling - will block but works!
            self.scroll_text_simple(line1, message_preview, cycles=3)
        else:
            print(f"Static display (message: {len(message_preview)} chars)")
...
def button_callback(channel):
    ...
    print(f"\n{'='*70}")
    print(f"[{timestamp}] 🔘 BUTTON PRESSED! (Channel: {channel})")
    print(f"{'='*70}")
    print(f"DEBUG: meshtastic_interface = {meshtastic_interface}")
    print(f"DEBUG: node_name = {node_name}")
    print(f"DEBUG: alert_active = {alert_active}")
    print(f"DEBUG: last_alert_message length = {len(last_alert_message) if last_alert_message else 0}")
    
    if lcd:
        try:
            lcd.show_button_press()
        except Exception as e:
            print(f"LCD show_button_press error: {e}")
    ...
    try:
        if meshtastic_interface:
            ...
            if alert_active and lcd:
                alert_type, full_msg = extract_alert_info(last_alert_message)
                lcd.show_alert(alert_type, full_msg, scroll=False)  # No scroll on return
            elif lcd:
                lcd.show_ready()
        else:
            print("✗ Cannot send - Meshtastic interface is None")
            print(f"   Interface object: {meshtastic_interface}")
...
def on_receive(packet, interface):
    ...
    try:
        ...
        if alert_type == "ALL CLEAR":
            ...
            # Update LCD
            print(f"DEBUG: Calling lcd.show_alert for ALL CLEAR")
            if lcd and lcd.enabled:
                lcd.show_alert(alert_type, full_message, scroll=False)
            else:
                print(f"⚠ LCD not available (lcd={lcd}, enabled={lcd.enabled if lcd else 'N/A'})")
            ...
            print(f"DEBUG: Full message length: {len(full_message)} chars")
            print(f"DEBUG: Message preview: {full_message[:60]}...")
            print(f"DEBUG: LCD object: {lcd}")
            print(f"DEBUG: LCD enabled: {lcd.enabled if lcd else 'N/A'}")
            
            # Update LCD with scrolling for long messages
            print(f"DEBUG: Calling lcd.show_alert with scroll=True")
            if lcd and lcd.enabled:
                try:
                    lcd.show_alert(alert_type, full_message, scroll=True)
                    print(f"DEBUG: lcd.show_alert completed")
                except Exception as e:
                    print(f"✗ LCD show_alert error: {e}")
                    import traceback
                    traceback.print_exc()
            else:
                print(f"⚠ LCD not available - cannot display alert")
...
def main():
    ...
    # Check button (polling mode - no interrupts)
    if GPIO_AVAILABLE:
        try:
            button_state = GPIO.input(BUTTON_PIN)
            print(f"✓ Button configured on GPIO {BUTTON_PIN} (polling mode)")
            print(f"  Current state: {button_state} ({'HIGH/not pressed' if button_state else 'LOW/pressed'})")
        except Exception as e:
    ...
    try:
        ...
        print(f"Button ACK: ENABLED on GPIO {BUTTON_PIN} (polling mode)" if GPIO_AVAILABLE else "Button ACK: DISABLED")
        print("\nFeatures:")
        ...
        button_last_check = time.time()
        
        # Keep running with button polling
        while True:
            # Check button state every 100ms
            current_time = time.time()
            if GPIO_AVAILABLE and (current_time - button_last_check) > 0.1:
                button_last_check = current_time
                button_state = GPIO.input(BUTTON_PIN)
